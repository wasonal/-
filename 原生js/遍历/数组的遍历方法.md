...
### reduce
#### 基本介绍
reduce方法按升序对数组中每个元素执行callback，将accumulator赋值为callback返回结果，返回最后的accumulator为reduce返回值    
```
let arr = [1,2,3];
//对arr数组求和
...
4. 实现的思路为len初始化，for实现遍历，in判断是否初始化，fn.call调用函数
